...
bool DrawLynxFastImageElementTextureSurface(
...
class LynxFastImageElementTextureView : public LynxFastImageElementView {
 public:
  explicit LynxFastImageElementTextureView(void* opaque)
 ...
 private:
  ...
  void Render() {
    ...
    if (DrawLynxFastImageElementTextureSurface(
            surface,
            width_px_,
            height_px_,
            value().c_str())) {
...
lynx_native_view_t* CreateLynxFastImageElementTextureNativeView(void* opaque) {
  auto* view = new LynxFastImageElementTextureView(opaque);
